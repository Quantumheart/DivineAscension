...
public class BlessingCommandAdminTests : BlessingCommandsTestHelpers
{
    ...
    public void OnAdminUnlock_PlayerBlessing_UnlocksSuccessfully()
    {
        ...
        _blessingRegistry.Setup(r => r.GetBlessing("khoras_strength")).Returns(blessing);
        ...
        _blessingEffectSystem.Verify(s => s.RefreshPlayerBlessings("player-1"), Times.Once);
    }
    ...
    public void OnAdminUnlock_PlayerBlessing_TargetSelf()
    {
        ...
        // Setup mocks
        _playerReligionDataManager.Setup(m => m.GetOrCreatePlayerData("admin-1")).Returns(playerData);
        _religionManager.Setup(m => m.GetPlayerActiveDeityDomain("admin-1")).Returns(DeityDomain.Wild);
        _religionManager.Setup(m => m.GetPlayerReligion("admin-1")).Returns(religion);
        _blessingRegistry.Setup(r => r.GetBlessing("lysa_healing")).Returns(blessing);
    ...
    public void OnAdminUnlock_BlessingNotFound_ReturnsError()
    {
        ...
        _playerReligionDataManager.Setup(m => m.GetOrCreatePlayerData("player-1")).Returns(playerData);
        _religionManager.Setup(m => m.GetPlayerActiveDeityDomain("player-1")).Returns(DeityDomain.Craft);
        _blessingRegistry.Setup(r => r.GetBlessing("invalid_blessing"))
    ...
    public void OnAdminUnlockAll_UnlocksAllPlayerBlessings()
    {
        ...
        _religionManager.Setup(m => m.GetPlayerActiveDeityDomain("player-1")).Returns(DeityDomain.Craft);
        _religionManager.Setup(m => m.GetPlayerReligion("player-1")).Returns(religion);
        _blessingRegistry.Setup(r => r.GetBlessingsForDeity(DeityDomain.Craft, BlessingKind.Player))
            .Returns(playerBlessings);
        _blessingRegistry.Setup(r => r.GetBlessingsForDeity(DeityDomain.Craft, BlessingKind.Religion))
