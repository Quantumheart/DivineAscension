...
public class CooldownManagerTests
{
    ...
    private readonly Mock<IServerEventAPI> _mockEvent;
    ...
    public CooldownManagerTests()
    {
        ...
        _mockEvent = new Mock<IServerEventAPI>();
        _config = new ModConfigData();
        _currentTimeMs = 0;

        _mockAPI.Setup(a => a.Logger).Returns(_mockLogger.Object);
        _mockAPI.Setup(a => a.World).Returns(_mockWorld.Object);
        _mockAPI.Setup(a => a.Event).Returns(_mockEvent.Object);
        _mockWorld.Setup(w => w.ElapsedMilliseconds).Returns(() => _currentTimeMs);

        _cooldownManager = new CooldownManager(_mockAPI.Object, _config);
    }
    ...
    public void CanPerformOperation_WithNoCooldown_AllowsOperation()
    {
        ...
        // Act
        var allowed = _cooldownManager.CanPerformOperation(playerUID, CooldownType.ReligionDeletion, out var errorMessage);
    ...
    public void CanPerformOperation_AdminPlayer_BypassesCooldown()
    {
        ...
        _mockWorld.Setup(w => w.PlayerByUid(adminUID)).Returns(mockAdminPlayer.Object);

        _cooldownManager.RecordOperation(adminUID, CooldownType.ReligionDeletion);

        // Act - Admin should bypass cooldown
        var allowed = _cooldownManager.CanPerformOperation(adminUID, CooldownType.ReligionDeletion, out var errorMessage);
    ...
    public void TryPerformOperation_WithActiveCooldown_Fails()
    {
        ...
        // Act
        var success = _cooldownManager.TryPerformOperation(playerUID, CooldownType.WarDeclaration, out var errorMessage);
    ...
    public void GetRemainingCooldown_ActiveCooldown_ReturnsCorrectValue()
    {
        ...
        // Act
        _currentTimeMs += 3000; // 3 seconds elapsed
    ...
    public void GetCooldownDuration_RespectsCustomConfig()
    {
        ...
        var customManager = new CooldownManager(_mockAPI.Object, customConfig);
    ...
    public void CleanupExpiredCooldowns_RemovesExpiredCooldowns()
    {
        ...
        long cleanupCallbackId = 0;
        _mockEvent.Setup(e => e.RegisterCallback(It.IsAny<Action<float>>(), It.IsAny<int>()))
            .Callback<Action<float>, int>((callback, interval) =>
            {
                cleanupCallbackId = 123; // Mock callback ID

                // Simulate cleanup after 5 seconds
                _currentTimeMs += 5000;
                callback(0); // Call the cleanup manually
            })
            .Returns(() => cleanupCallbackId);
    ...
    public void Initialize_RegistersCleanupCallback()
    {
        ...
        // Assert
        _mockEvent.Verify(e => e.RegisterCallback(It.IsAny<Action<float>>(), 300000), Times.Once);
    ...
    {
        // Arrange
        long mockCallbackId = 123;
        _mockEvent.Setup(e => e.RegisterCallback(It.IsAny<Action<float>>(), It.IsAny<int>()))
            .Returns(mockCallbackId);
        _cooldownManager.Initialize();
        ...
        // Assert
        _mockEvent.Verify(e => e.UnregisterCallback(mockCallbackId), Times.Once);
    }
