using System;
using System.Collections.Generic;
using System.Text;
using System.Text.Json;
using DivineAscension.Constants;
using DivineAscension.Models;
using DivineAscension.Models.Dto;
using DivineAscension.Models.Enum;
using DivineAscension.Services.Interfaces;
using Vintagestory.API.Common;

namespace DivineAscension.Services;

/// <summary>
///     Loads blessing definitions from JSON asset files.
///     Follows the LocalizationService pattern for asset loading.
/// </summary>
public class BlessingLoader : IBlessingLoader
{
    private static readonly string[] DomainFiles = { "craft", "wild", "conquest", "harvest", "stone" };

    private readonly ICoreAPI _api;
    private readonly List<Blessing> _loadedBlessings = new();
    private readonly ILoggerWrapper _logger;

    public BlessingLoader(ICoreAPI api, ILoggerWrapper logger)
    {
        _api = api ?? throw new ArgumentNullException(nameof(api));
        _logger =  logger ?? throw new ArgumentNullException(nameof(logger));
    }

    /// <inheritdoc />
    public bool LoadedSuccessfully { get; private set; }

    /// <inheritdoc />
    public int LoadedCount => _loadedBlessings.Count;

    /// <inheritdoc />
    public List<Blessing> LoadBlessings()
    {
        _loadedBlessings.Clear();
        LoadedSuccessfully = false;

        var totalLoaded = 0;
        var filesLoaded = 0;

        foreach (var domainFile in DomainFiles)
        {
            try
            {
                var blessingsFromFile = LoadBlessingsFromFile(domainFile);
                if (blessingsFromFile.Count > 0)
                {
                    _loadedBlessings.AddRange(blessingsFromFile);
                    totalLoaded += blessingsFromFile.Count;
                    filesLoaded++;
                    _logger.Debug(
                        $"[DivineAscension BlessingLoader] Loaded {blessingsFromFile.Count} blessings from {domainFile}.json");
                }
            }
            catch (Exception ex)
            {
                _logger.Error(
                    $"[DivineAscension BlessingLoader] Failed to load blessings from {domainFile}.json: {ex.Message}");
            }
        }

        if (filesLoaded > 0)
        {
            LoadedSuccessfully = true;
            _logger.Notification(
                $"[DivineAscension BlessingLoader] Successfully loaded {totalLoaded} blessings from {filesLoaded} files");
        }
        else
        {
            _logger.Warning(
                "[DivineAscension BlessingLoader] No blessing files were loaded. Fallback to hardcoded definitions may be needed.");
        }

        return new List<Blessing>(_loadedBlessings);
    }

    /// <summary>
    ///     Loads blessings from a specific domain JSON file.
    /// </summary>
    private List<Blessing> LoadBlessingsFromFile(string domainFileName)
    {
        var blessings = new List<Blessing>();
        var assetPath = $"config/blessings/{domainFileName}.json";

        var asset = _api.Assets.Get(new AssetLocation("divineascension", assetPath));
        if (asset == null)
        {
            _logger.Debug($"[DivineAscension BlessingLoader] Asset not found: {assetPath}");
            return blessings;
        }

        var json = Encoding.UTF8.GetString(asset.Data);
        var fileDto = JsonSerializer.Deserialize<BlessingFileDto>(json, new JsonSerializerOptions
        {
            PropertyNameCaseInsensitive = true
        });

        if (fileDto == null)
        {
            _logger.Warning($"[DivineAscension BlessingLoader] Failed to parse {domainFileName}.json");
            return blessings;
        }

        // Validate and convert domain
        if (!TryParseDomain(fileDto.Domain, out var domain))
        {
            _logger.Error(
                $"[DivineAscension BlessingLoader] Invalid domain '{fileDto.Domain}' in {domainFileName}.json");
            return blessings;
        }

        foreach (var dto in fileDto.Blessings)
        {
            try
            {
                var blessing = ConvertToBlessing(dto, domain);
                if (blessing != null)
                {
                    blessings.Add(blessing);
                }
            }
            catch (Exception ex)
            {
                _logger.Warning(
                    $"[DivineAscension BlessingLoader] Failed to convert blessing '{dto.BlessingId}': {ex.Message}");
            }
        }

        return blessings;
    }

    /// <summary>
    ///     Converts a BlessingJsonDto to a Blessing model.
    /// </summary>
    private Blessing? ConvertToBlessing(BlessingJsonDto dto, DeityDomain domain)
    {
        // Validate required fields
        if (string.IsNullOrWhiteSpace(dto.BlessingId))
        {
            _logger.Warning("[DivineAscension BlessingLoader] Blessing has empty BlessingId, skipping");
            return null;
        }

        if (string.IsNullOrWhiteSpace(dto.Name))
        {
            _logger.Warning($"[DivineAscension BlessingLoader] Blessing '{dto.BlessingId}' has empty Name, skipping");
            return null;
        }

        // Parse Kind enum
        if (!TryParseKind(dto.Kind, out var kind))
        {
            _logger.Warning(
                $"[DivineAscension BlessingLoader] Blessing '{dto.BlessingId}' has invalid Kind '{dto.Kind}', skipping");
            return null;
        }

        // Parse Category enum
        if (!TryParseCategory(dto.Category, out var category))
        {
            _logger.Warning(
                $"[DivineAscension BlessingLoader] Blessing '{dto.BlessingId}' has invalid Category '{dto.Category}', using Utility as default");
            category = BlessingCategory.Utility;
        }

        var blessing = new Blessing(dto.BlessingId, dto.Name, domain)
        {
            Description = dto.Description ?? string.Empty,
            Kind = kind,
            Category = category,
            IconName = dto.IconName ?? string.Empty,
            RequiredFavorRank = dto.RequiredFavorRank,
            RequiredPrestigeRank = dto.RequiredPrestigeRank,
            PrerequisiteBlessings = dto.PrerequisiteBlessings ?? new List<string>(),
            StatModifiers = dto.StatModifiers ?? new Dictionary<string, float>(),
            SpecialEffects = dto.SpecialEffects ?? new List<string>(),
            Cost = dto.Cost,
            Branch = dto.Branch,
            ExclusiveBranches = dto.ExclusiveBranches,
            RequiresPatron = dto.RequiresPatron
        };

        // Log warning for unknown stat keys (but still include them)
        if (dto.StatModifiers != null)
        {
            foreach (var key in dto.StatModifiers.Keys)
            {
                if (!IsKnownStatKey(key))
                {
                    _logger.Debug(
                        $"[DivineAscension BlessingLoader] Blessing '{dto.BlessingId}' has unknown stat key '{key}'");
                }
            }
        }

        return blessing;
    }

    /// <summary>
    ///     Tries to parse a domain string to DeityDomain enum.
    /// </summary>
    private static bool TryParseDomain(string domainStr, out DeityDomain domain)
    {
        return Enum.TryParse(domainStr, ignoreCase: true, out domain);
    }

    /// <summary>
    ///     Tries to parse a kind string to BlessingKind enum.
    /// </summary>
    private static bool TryParseKind(string kindStr, out BlessingKind kind)
    {
        return Enum.TryParse(kindStr, ignoreCase: true, out kind);
    }

    /// <summary>
    ///     Tries to parse a category string to BlessingCategory enum.
    /// </summary>
    private static bool TryParseCategory(string categoryStr, out BlessingCategory category)
    {
        return Enum.TryParse(categoryStr, ignoreCase: true, out category);
    }

    /// <summary>
    ///     Checks if a stat key is known in VintageStoryStats.
    ///     This is a best-effort check using reflection.
    /// </summary>
    private static bool IsKnownStatKey(string key)
    {
        // Check against known stat keys from VintageStoryStats
        // This uses a HashSet for O(1) lookup
        return KnownStatKeys.Contains(key);
    }

    /// <summary>
    ///     Set of all known stat keys from VintageStoryStats for validation.
    /// </summary>
    private static readonly HashSet<string> KnownStatKeys = new()
    {
        // Combat Stats
        VintageStoryStats.MeleeWeaponsDamage,
        VintageStoryStats.RangedWeaponsDamage,
        VintageStoryStats.MeleeWeaponsSpeed,
        VintageStoryStats.RangedWeaponsAccuracy,
        VintageStoryStats.RangedWeaponsRange,

        // War Stats
        VintageStoryStats.KillHealthRestore,
        VintageStoryStats.DamageReduction,
        VintageStoryStats.CriticalHitChance,
        VintageStoryStats.CriticalHitDamage,

        // Defense Stats
        VintageStoryStats.MeleeWeaponArmor,
        VintageStoryStats.MaxHealthExtraPoints,
        VintageStoryStats.MaxHealthExtraMultiplier,
        VintageStoryStats.ArmorEffectiveness,

        // Movement Stats
        VintageStoryStats.WalkSpeed,

        // Utility Stats
        VintageStoryStats.HealingEffectiveness,

        // Craft Stats
        VintageStoryStats.ToolDurability,
        VintageStoryStats.OreDropRate,
        VintageStoryStats.ColdResistance,
        VintageStoryStats.MiningSpeed,
        VintageStoryStats.RepairCostReduction,
        VintageStoryStats.RepairEfficiency,
        VintageStoryStats.SmithingCostReduction,
        VintageStoryStats.MetalArmorBonus,
        VintageStoryStats.HungerRate,
        VintageStoryStats.ArmorDurabilityLoss,
        VintageStoryStats.ArmorWalkSpeedAffectedness,

        // Wild Stats
        VintageStoryStats.DoubleHarvestChance,
        VintageStoryStats.AnimalDamage,
        VintageStoryStats.AnimalDrops,
        VintageStoryStats.ForageDropRate,
        VintageStoryStats.FoodSpoilage,
        VintageStoryStats.Satiety,
        VintageStoryStats.TemperatureResistance,
        VintageStoryStats.AnimalHarvestTime,
        VintageStoryStats.ForagingYield,

        // Harvest Stats
        VintageStoryStats.CropYield,
        VintageStoryStats.SeedDropChance,
        VintageStoryStats.CookingYield,
        VintageStoryStats.RareCropChance,
        VintageStoryStats.CookedFoodSatiety,

        // Stone Stats
        VintageStoryStats.StoneYield,
        VintageStoryStats.ClayYield,
        VintageStoryStats.ClayFormingVoxelChance,
        VintageStoryStats.PotteryBatchCompletionChance,
        VintageStoryStats.StorageVesselCapacity,
        VintageStoryStats.DiggingSpeed,
        VintageStoryStats.PickDurability,
        VintageStoryStats.FallDamageReduction,
        VintageStoryStats.RareStoneChance,
        VintageStoryStats.OreInStoneChance,
        VintageStoryStats.GravelYield,

        // Other
        VintageStoryStats.AnimalSeekingRange,

        // CombatOverhaul Stats
        VintageStoryStats.MeleeDamageTierBonusSlashing,
        VintageStoryStats.MeleeDamageTierBonusPiercing,
        VintageStoryStats.MeleeDamageTierBonusBlunt,
        VintageStoryStats.RangedDamageTierBonusSlashing,
        VintageStoryStats.RangedDamageTierBonusPiercing,
        VintageStoryStats.RangedDamageTierBonusBlunt,
        VintageStoryStats.ArmorManipulationSpeedAffectedness,
        VintageStoryStats.ArmorHungerRateAffectedness,
        VintageStoryStats.ManipulationSpeed,
        VintageStoryStats.SteadyAim,
        VintageStoryStats.MechanicalsDamage,
        VintageStoryStats.PlayerHeadDamageFactor,
        VintageStoryStats.PlayerFaceDamageFactor,
        VintageStoryStats.PlayerNeckDamageFactor,
        VintageStoryStats.PlayerTorsoDamageFactor,
        VintageStoryStats.PlayerArmsDamageFactor,
        VintageStoryStats.PlayerLegsDamageFactor,
        VintageStoryStats.PlayerHandsDamageFactor,
        VintageStoryStats.PlayerFeetDamageFactor,
        VintageStoryStats.BowsProficiency,
        VintageStoryStats.CrossbowsProficiency,
        VintageStoryStats.FirearmsProficiency,
        VintageStoryStats.OneHandedSwordsProficiency,
        VintageStoryStats.TwoHandedSwordsProficiency,
        VintageStoryStats.SpearsProficiency,
        VintageStoryStats.JavelinsProficiency,
        VintageStoryStats.MacesProficiency,
        VintageStoryStats.ClubsProficiency,
        VintageStoryStats.HalberdsProficiency,
        VintageStoryStats.AxesProficiency,
        VintageStoryStats.QuarterstaffProficiency,
        VintageStoryStats.SlingsProficiency,
        VintageStoryStats.SecondChanceCooldown,
        VintageStoryStats.SecondChanceGracePeriod
    };
}
