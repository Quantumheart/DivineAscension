...
public static class TabControl
{
    ...
    /// <summary>
    ///     Draw a horizontal tab control with hover state tracking
    /// </summary>
    /// <param name="drawList">ImGui draw list</param>
    /// <param name="x">X position</param>
    /// <param name="y">Y position</param>
    ...
    public static (int selectedIndex, int hoveredIndex) DrawWithHover(
        ...
        string[]? iconNames = null)
    {
        if (tabs == null || tabs.Length == 0) return (selectedIndex, -1);
        ...
        for (var i = 0; i < tabs.Length; i++)
        {
            ...
            var (clicked, hovering) = DrawTabWithHoverTracking(
                drawList, tabs[i], tabX, y, tabWidth, height,
    ...
    /// <summary>
    ///     Draw a single tab with hover state tracking
    /// </summary>
    /// <returns>Tuple of (clicked, hovering)</returns>
    private static (bool clicked, bool hovering) DrawTabWithHoverTracking(
        ImDrawListPtr drawList,
        string text,
        float x,
        float y,
