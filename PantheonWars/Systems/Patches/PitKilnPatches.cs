...
public static class PitKilnPatches
{
    ...
    public static void PostFix_TryIgnite(BlockEntityPitKiln __instance, IPlayer? byPlayer)
    {
        ...
        {
            _kilnOwners.AddOrUpdate(__instance, byPlayer.PlayerUID);
        }
    }
    ...
    public static void Postfix_FromTreeAttributes(BlockEntityPitKiln __instance, ITreeAttribute tree)
    {
        ...
        {
            _kilnOwners.AddOrUpdate(__instance, uid);
        }
    }
    ...
    public static void Postfix_OnFired(BlockEntityPitKiln __instance, List<ItemStack> __state)
    {
        ...
        string cycleKey = $"{__instance.Pos.X},{__instance.Pos.Y},{__instance.Pos.Z}@{__instance.BurningUntilTotalHours.ToString("F3", CultureInfo.InvariantCulture)}";
        if (!_completedCycles.Add(cycleKey))
        {
            __instance.Api.Logger.Warning($"[PantheonWars] Prevented duplicate Pit Kiln firing event at {__instance.Pos} for cycle {__instance.BurningUntilTotalHours:F3}");
            return;
        }
        ...
        if (firedItems.Count > 0)
        {
            ...
            {
                // Log debug info
                __instance.Api.Logger.Debug($"[PantheonWars] PitKilnPatches: Firing event for {uid} with {firedItems.Count} items at {__instance.Pos}");
