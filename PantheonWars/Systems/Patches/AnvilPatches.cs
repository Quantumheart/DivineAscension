...
public static class AnvilPatches
{
    ...
    [HarmonyPrefix]
    public static void Prefix_OnHammerHitOver(BlockEntityAnvilPart __instance, IPlayer byPlayer, Vec3d hitPosition, out CompletionState __state)
    {
        __state = default;
    ...
    [HarmonyPatch(typeof(BlockEntityAnvilPart), "OnHammerHitOver")]
    [HarmonyPostfix]
    public static void Postfix_OnHammerHitOver(BlockEntityAnvilPart __instance, IPlayer byPlayer, Vec3d hitPosition, CompletionState __state)
    {
        var api = __instance.Api;
        if (api == null || api.Side != EnumAppSide.Server) return;
        ...
        bool hasWorkNow = be.WorkItemStack != null;
        if (!hasWorkNow)
        {
    ...
    [HarmonyPrefix]
    public static void Prefix_CheckIfFinished(BlockEntityAnvil __instance, IPlayer byPlayer, out CompletionState __state)
    {
        __state = default;
        var api = __instance.Api;
        if (api == null || api.Side != EnumAppSide.Server) return;
    ...
    public static void Postfix_CheckIfFinished(BlockEntityAnvil __instance, IPlayer byPlayer, CompletionState __state)
    {
        ...
        if (!__state.HadWorkItem) return;
        bool hasWorkNow = __instance.WorkItemStack != null;
