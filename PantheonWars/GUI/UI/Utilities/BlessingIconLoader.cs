...
internal static class BlessingIconLoader
{
    ...
    private static LoadedTexture? LoadBlessingTexture(DeityType deity, string iconName)
    {
        ...
        try
        {
            ...
            if (asset == null)
            {
                _api.Logger.Warning($"[PantheonWars] Blessing icon not found: {assetPath}");
                return null;
            }
            ...
            if (textureId == 0)
            {
                _api.Logger.Warning($"[PantheonWars] Failed to load blessing texture: {assetPath}");
                return null;
            }
            ...
            _api.Logger.Debug(
                $"[PantheonWars] Loaded blessing icon: {deityName}/{normalizedIconName} (ID: {texture.TextureId})");
            return texture;
        }
        catch (Exception ex)
        {
            _api.Logger.Error(
                $"[PantheonWars] Error loading blessing texture {deityName}/{normalizedIconName}: {ex.Message}");
            return null;
        }
    }
