...
internal static class CivilizationIconLoader
{
    ...
    private static LoadedTexture? LoadCivilizationTexture(string iconName)
    {
        ...
        try
        {
            ...
            if (asset == null)
            {
                _api.Logger.Warning($"[PantheonWars] Civilization icon not found: {assetPath}");

                // Try to load default icon as fallback
                if (normalizedIconName != "default")
                {
                    _api.Logger.Debug("[PantheonWars] Attempting to load default civilization icon as fallback");
            ...
            if (textureId == 0)
            {
                _api.Logger.Warning($"[PantheonWars] Failed to load civilization texture: {assetPath}");
            ...
            _api.Logger.Debug(
                $"[PantheonWars] Loaded civilization icon: {normalizedIconName} (ID: {texture.TextureId})");
            return texture;
        }
        catch (Exception ex)
        {
            _api.Logger.Error($"[PantheonWars] Error loading civilization texture {normalizedIconName}: {ex.Message}");
